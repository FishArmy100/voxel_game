...
use self::renderer::BindGroupData;
...
impl Renderer
{
    pub fn new(device: Arc<wgpu::Device>, surface: Arc<wgpu::Surface>, queue: Arc<wgpu::Queue>, config: &wgpu::SurfaceConfiguration) -> Self
    {
        ...
        let camera_bind_group = BindGroupData::uniform("camera_bind_group".into(), CameraUniform::new(), wgpu::ShaderStages::VERTEX, &device);
        let model_bind_group = BindGroupData::uniform("model_bind_group".into(), ModelUniform::from_position((0.0, 0.0, 0.0).into()), wgpu::ShaderStages::VERTEX, &device);

        let render_voxel_bind_group_layout = BindGroupData::get_uniform_layout(wgpu::ShaderStages::VERTEX, &device);

        let depth_texture = Texture::create_depth_texture(&device, &config, "depth_texture");

        let render_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Render Pipeline Layout"),
            bind_group_layouts: &[&camera_bind_group.layout(), &model_bind_group.layout(), &render_voxel_bind_group_layout],
    ...
    pub fn render<const N: usize>(&mut self, camera: &Camera, faces: &[VoxelFaceData], render_voxels: VoxelRenderDataUniform<N>, transform: ModelUniform) -> Result<(), wgpu::SurfaceError>
    {
        ...
        let render_voxel_bind_group = BindGroupData::uniform("render_voxel_bind_group".into(), render_voxels, wgpu::ShaderStages::VERTEX, &self.device);
        ...
        for slice in &ranges
        {
            self.model_bind_group.enqueue_set_data(&self.queue, transform);

            let mut camera_uniform = CameraUniform::new();
            camera_uniform.update_view_proj(camera);
            self.camera_bind_group.enqueue_set_data(&self.queue, camera_uniform);
            ...
            render_pass.set_bind_group(1, self.model_bind_group.bind_group(), &[]);
            render_pass.set_bind_group(2, render_voxel_bind_group.bind_group(), &[]);
